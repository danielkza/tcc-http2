...
%
\documentclass[$if(fontsize)$$fontsize$,$endif$$if(papersize)$$papersize$,$endif$$for(classoption)$$classoption$$sep$,$endfor$,$if(lang)$$lang$,$endif$]{$documentclass$}	% Opções da classe do documento:
...
\else
  \usepackage[]{babel}	% Adiciona a última língua de $lang$
\fi
$endif$
$for(header-includes)$
$header-includes$
...
\orientador{$for(orientador)$$orientador$$sep$ \and $endfor$}  % Adicionando orientadores(es)
...
$if(resumo)$
\begin{resumo}
$resumo$   % Adicionando o resumo (Português)
\\
$if(tags)$
\vspace{\onelineskip}
\noindent\textbf{Palavras-chave}: $for(palavraschave)$$palavraschave$$sep$. $endfor$.  % Adicionando as palavras-chaves (Português)
$endif$
\end{resumo}
$endif$
...
$abstract$   % Adicionando o resumo (Inglês)
\\
$if(tags)$
\vspace{\onelineskip}
\noindent\textbf{Keywords}: $for(tags)$$tags$$sep$. $endfor$.  % Adicionando as palavras-chaves (Inglês)
$endif$
\end{otherlanguage*}
\end{resumo}
$endif$
